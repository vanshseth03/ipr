...
function detectQueryLanguage(text) {
  ...
  const handleSend = async (text) => {
    ...
    for (const m of validHistoryMessages.slice(-4)) {
    ...
    const outgoingQuery = fullPrompt;
  ...
  const handleSpeakMessage = useCallback(
    (message) => {
      ...
      let msgLang = 'en-IN';
      const isHindi = /[\u0900-\u097F]/.test(content) || language === 'hi' || detectQueryLanguage(content) === 'hi';
      const isTamil = /[\u0B80-\u0BFF]/.test(content) || language === 'ta';
      const isTelugu = /[\u0C00-\u0C7F]/.test(content) || language === 'te';
      const isBengali = /[\u0980-\u09FF]/.test(content) || language === 'bn';
      ...
      speak(content, { language: msgLang }, message.id);
    },
