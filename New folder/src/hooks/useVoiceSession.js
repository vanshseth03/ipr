...
export function useVoiceSession() {
  ...
  const processStreamingTalkback = async (userText, detectedLanguage) => {
    ...
    let targetLang = 'en';
    if (/[\u0900-\u097F]/.test(userText) || detectedLanguage === 'hi' || currentLanguage === 'hi') {
      targetLang = 'hi';
    } else if (/[\u0B80-\u0BFF]/.test(userText) || detectedLanguage === 'ta' || currentLanguage === 'ta') {
      targetLang = 'ta';
    } else if (/[\u0C00-\u0C7F]/.test(userText) || detectedLanguage === 'te' || currentLanguage === 'te') {
      targetLang = 'te';
    } else if (/[\u0980-\u09FF]/.test(userText) || detectedLanguage === 'bn' || currentLanguage === 'bn') {
      targetLang = 'bn';
