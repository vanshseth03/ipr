...
class LLMManager:
    ...
    def prepare_inputs(self, system_prompt: str, user_message: str, is_hindi: bool = False, is_out_of_domain: bool = False, history: Optional[List[Dict]] = None, is_translation: bool = False, target_lang: str = "hi"):
        """Prepare chat template token inputs with clean context isolation."""
        ...
        if history and len(history) > 0:
            last_role = None
            import re
            # Only keep last 3 turns to prevent context bleed between conversations
            for turn in history[-3:]:
                ...
                if r == last_role:
                    ...
                    # Strip noise: disclaimers, citation scores, excessive whitespace
                    clean_c = re.sub(r'Disclaimer:.*$', '', content, flags=re.IGNORECASE|re.MULTILINE)
                    clean_c = re.sub(r'\[\d+\].*?relevance:[0-9.\-]+', '', clean_c)
                    clean_c = re.sub(r'\s+', ' ', clean_c).strip()
                    # Compress history turns to 300 chars — enough for context, not enough to pollute
                    messages.append({"role": r, "content": clean_c[:300]})
                    last_role = r

            # Ensure history ends on model turn so new query is user turn
            while messages and messages[-1]["role"] == "user":
                messages.pop()

        # Gemma format: merge system prompt + query into user message
        if "gemma" in str(self.model_name).lower() or "gemma" in str(self.model_id).lower():
            if is_translation:
                if target_lang == "hi" or "hindi" in target_lang.lower() or "हिंदी" in target_lang:
                    turn = f"उपयोगकर्ता: {user_message}\n\nनिर्देश: पिछले उत्तर का पूर्ण हिंदी अनुवाद दें। कोई नया विषय न जोड़ें।"
                else:
...
class OmniVoiceTTSManager:
    ...
    def synthesize(self, text: str, language: str = "auto") -> bytes:
        """Convert text to speech using OmniVoice. Generates complete text in one go. Returns WAV bytes."""
        ...
        clean = re.sub(r'\bv\.\s+', 'versus ', clean, flags=re.IGNORECASE)
        clean = re.sub(r'\bvs\.\s*', 'versus ', clean, flags=re.IGNORECASE)
        clean = re.sub(r'\bSec\.\s*', 'Section ', clean, flags=re.IGNORECASE)
        ...
        for s in sentences:
            ...
            if len(s) > 400:
                ...
                for sp in sub_parts:
                    ...
                    if not sp:
                        continue
                    if len(curr) + len(sp) + 1 <= 400:
                        curr = f"{curr} {sp}".strip()
                    else:
                        if curr:
                            batches.append(curr)
                        curr = sp
            elif len(curr) + len(s) + 1 <= 400:
                curr = f"{curr} {s}".strip()
            else:
                if curr:
                    batches.append(curr)
        ...
        # Float [-1,1] -> int16 WAV
        audio_int16 = np.clip(combined_audio, -1.0, 1.0)
...
def format_statutory_context(context_text: str, is_hi: bool = False) -> str:
    """Format statutory context block. If empty, provides clean instructions without hallucinated acts."""
...
def is_ayush_ipr_query(query: str) -> bool:
    ...
    domain_stems = [
        ...
        'chyawanprash', 'bhasma', 'churna', 'rasayana', 'kwath', 'taila', 'asava', 'arishta',
    ...
    # Queries with no domain keywords are out of domain
    return False


def build_context_string(results: List[Dict], min_score: float = -2.0) -> str:
    """Build formatted, saturated context string from search results.
    Filters out low-confidence noise chunks to prevent RAG context contamination."""
    context_parts = []
    # Filter by cross-encoder rerank score or similarity score
    valid = []
    for r in results:
        score = r.get("rerank_score", r.get("similarity_score", 0))
        # If cross-encoder ran, score < -2.0 indicates irrelevant chunk
    ...
    # If no results meet strict threshold, check if top score is reasonably close
    if not valid and results:
        best_score = results[0].get("rerank_score", -999)
        if best_score > -2.8:
            valid = results[:2]

    # Limit to top 3-4 saturated results
    for i, r in enumerate(valid[:4], 1):
        citation = r.get("rag_config", {}).get("citation_format", r.get("doc_id", ""))
        title = r.get("title", "")
        # Compact saturated window (700 chars) focused on operative legal text
        content = r.get("content", "")[:700].strip()
        # Clean file-internal figure/illustration/table references so LLM never parrots non-existent elements
        content = re.sub(r'\(?\b(?:see|refer to|in)\s+(?:illustration|fig(?:ure)?|table|diagram|chart)\s*\d+[^)]*\)?', '', content, flags=re.IGNORECASE)
        content = re.sub(r'\b(?:illustration|figure|diagram|table)\s*\d+[:\-.]?', '', content, flags=re.IGNORECASE)
        rerank = r.get("rerank_score", r.get("similarity_score", 0))

        context_parts.append(
            f"[{i}] {citation} | {title} (relevance:{rerank:.2f})\n{content}\n"
        )
    return "\n".join(context_parts)
...
def estimate_max_tokens(query: str) -> int:
    ...
    # Intent signals for SHORT answers (factual lookups, yes/no, definitions)
    short_signals = ['what is', 'kya hai', 'define', 'meaning', 'full form', 'shelf life',
                     'who is', 'when was', 'which section', 'kon sa', 'kitna', 'how much',
    ...
    # Question mark with few words = likely factual
    if wc <= 5 and '?' in q:
        return 400
    if is_short_intent and wc <= 12:
        return 500
    if is_short_intent:
        return 700
    if is_long_intent:
...
def rag_query(query: str, rag_db: RAGDatabase, llm: LLMManager, language: Optional[str] = None, history: Optional[List[Dict]] = None) -> dict:
    ...
    if is_translation:
        ...
        # Step 1: Hybrid search → rerank (top-4 saturated context)
        ...
        prompt = sys_prompt.format(context=context_block)
